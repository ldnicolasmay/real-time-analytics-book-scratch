...
def generate_delivery_statuses(
    order, last_status_time, shop_location, delivery_location, points_to_generate
):
    statuses = []
    if points_to_generate > 0:
        try:
            extra_points = geoid.npts(
                shop_location[0],
                shop_location[1],
                delivery_location[0],
                delivery_location[1],
            ...
            for point in extra_points:
                next_status_time = last_status_time + timedelta(seconds=1)
    ...
    statuses.append(
        {
            "id": order["id"],
            "updatedAt": (last_status_time + timedelta(seconds=1)).isoformat(),
...
async def orders(stream):
    async for order in stream:
        ...
        await orders_statuses_topic.send(key=order_status["id"], value=order_status)
        ...
        for index in range(0, len(STATUSES)):
            ...
            next_status_time = last_status_time + timedelta(
        ...
        cursor.execute(
            "SELECT lat,lon FROM pizzashop.users WHERE id = %s", (order["userId"],)
        )
        ...
        next_status_time = last_status_time + timedelta(seconds=minutes_to_deliver * 60)
        other_statuses.add(
            {
                "id": order["id"],
                "updatedAt": next_status_time.isoformat(),
...
async def order_status_poller():
    statuses_to_publish = [
        ...
        if parser.parse(item["updatedAt"]) < datetime.now(tz=timezone.utc)
    ]
    for status in statuses_to_publish:
        await orders_statuses_topic.send(key=status["id"], value=status)
...
async def delivery_status_poller():
    ...
    for status in statuses_to_publish:
        await delivery_statuses_topic.send(key=status["id"], value=status)
