...
public class OrdersQueries {
    ...
    public OrdersSummary orderSummary() {
        ...
        KStreamsWindowStore<Long> totalOrdersStore = new KStreamsWindowStore<>(totalOrdersStore());
        ...
        long totalOrders = totalOrdersStore.firstEntry(oneHourAgo, now);
        ...
        return new OrdersSummary(currentTimePeriod, previousTimePeriod, totalOrders);
    }
    ...
    private ReadOnlyWindowStore<String, Long> totalOrdersStore() {
        while (true) {
            try {
                streams.start();
                return streams.store(StoreQueryParameters.fromNameAndType(
                    "TotalOrdersStore", QueryableStoreTypes.windowStore()
                ));
            } catch (InvalidStateStoreException e) {
                System.out.println("e = " + e);
                try {
